<template>
  <div class="product-keyword-candidates-container">
    ...
    <el-table :data="pageData.list" style="width: 100%" v-loading="loading">
      ...
      <el-table-column prop="productKeyword" label="候选关键词" min-width="220" show-overflow-tooltip />
      ...
      <el-table-column prop="firstSeen" label="首次出现" width="170">
        <template #default="{ row }">{{ fmtDate(row.firstSeen) }}</template>
      </el-table-column>
      <el-table-column prop="lastSeen" label="最后出现" width="170">
        <template #default="{ row }">{{ fmtDate(row.lastSeen) }}</template>
      </el-table-column>
    ...
    <el-dialog v-model="detailVisible" title="产品候选关键词详情" width="720px">
      <div v-loading="detailLoading">
        <el-descriptions v-if="detailData" :column="2" border>
          <el-descriptions-item label="ID">{{ detailData.id }}</el-descriptions-item>
          <el-descriptions-item label="候选关键词" :span="2">{{ detailData.productKeyword || '—' }}</el-descriptions-item>
          ...
          <el-descriptions-item label="首次出现">{{ fmtDate(detailData.firstSeen) }}</el-descriptions-item>
          <el-descriptions-item label="最后出现">{{ fmtDate(detailData.lastSeen) }}</el-descriptions-item>
          <el-descriptions-item label="统计日期" :span="2">{{ fmtDate(detailData.statDate) }}</el-descriptions-item>
        </el-descriptions>
      </div>
    </el-dialog>
  </div>
...
import { fetchProductKeywordCandidatePage, getProductKeywordCandidateById } from '@/api/product-keyword-candidate'
...
const pageData = reactive<PageResponse<ProductKeywordCandidateVO>>({
  pageNum: 1,
  pageSize: 20,
  total: 0,
  pages: 0,
...
const buildDto = (pageNum: number): ProductKeywordCandidatePageRequest => {
  return {
    pageNum,
...
const load = async (pageNum = 1) => {
  try {
    ...
    const res = await fetchProductKeywordCandidatePage(dto)
...
const detailData = ref<ProductKeywordCandidateVO | null>(null)

const openDetail = async (row: ProductKeywordCandidateVO) => {
  try {
    detailVisible.value = true
    detailLoading.value = true
    detailData.value = null
    const res = await getProductKeywordCandidateById(row.id)
    if (res.code === 0 && res.data) {
...
.product-keyword-candidates-container {
  padding: 24px;
