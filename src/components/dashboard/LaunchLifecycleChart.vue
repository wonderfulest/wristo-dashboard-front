...
const render=async()=>{await nextTick();if(!chartElement.value||!curve.value)return;chart ||= echarts.init(chartElement.value);const points=curve.value.points;const layout=buildBottomLineChartLayout();chart.setOption({tooltip:{trigger:'axis'},legend:{...layout.legend,data:['P25 下载','P50 下载','P75 下载','P50 营收']},grid:{left:50,right:55,top:42,...layout.grid},xAxis:{type:'category',name:'上线日龄',data:points.map(p=>p.ageDays)},yAxis:[{type:'value',name:'下载量',min:layout.yAxisMin},{type:'value',name:'营收($)',min:layout.yAxisMin,axisLabel:{formatter:(v:number)=>`$${v}`}}],series:[{name:'P25 下载',type:'line',symbol:'none',data:points.map(p=>p.p25Downloads),lineStyle:{color:'#aacfc0',type:'dashed'}},{name:'P50 下载',type:'line',symbol:'none',data:points.map(p=>p.p50Downloads),lineStyle:{color:'#16845a',width:3}},{name:'P75 下载',type:'line',symbol:'none',data:points.map(p=>p.p75Downloads),lineStyle:{color:'#75aa96',type:'dashed'}},{name:'P50 营收',type:'line',yAxisIndex:1,symbol:'none',data:points.map(p=>p.p50RevenueCents/100),lineStyle:{color:'#d18a22'}}]})}
