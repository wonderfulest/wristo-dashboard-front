...
test('data item editor exposes separate canonical labels, unit selection, and resource names', async () => {
  ...
  assert.match(pageSource, /:catalog-writable="catalogWritable"/)
  assert.match(pageSource, /if \(!catalogWritable\.value\) return/)
  assert.match(listSource, /:disabled="!catalogWritable \|\| activeLoadingIds\.has/)
  assert.match(listSource, /:disabled="!catalogWritable"/)
  assert.match(dialogSource, /validateFormWithCallback/)
  assert.match(dialogSource, /await submitRequest\(payload\)/)
